...
function styleNodeBodyParser(css, key, value, selector, viewName) {
  if (typeof key === 'string') {
    //mod condition
    if (key === '__ON' && typeof value === 'function') return value; //child

    if (Object.keys(types).includes(key[0])) {
      var _styleNodeParser = styleNodeParser(key, value, selector, viewName),
          child = _styleNodeParser.child,
          modCnd = _styleNodeParser.modCnd;
...
function styleNodeParser(key, content) {
  ...
  var _iterator2 = _createForOfIteratorHelper(contentEntries),
      _step2;

  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      var _step2$value = _slicedToArray(_step2.value, 2),
          childName = _step2$value[0],
          value = _step2$value[1];

      var r = styleNodeBodyParser(css, childName, value, selector, viewName);

      if (!r) {
        var childType = _typeof(childName);

        throw new Error("[JSVN] Incorrect key (".concat(childType, ") '").concat(childType === 'string' ? childName : '*', "' in node \"").concat(key, "\" (").concat(parentSelector, ") in view \"").concat(viewName, "\""));
