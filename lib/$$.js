...
function $$() {
  ...
  if (Array.isArray(args[0]) && typeof args[0][0] === 'string') {
    ...
    if (name.startsWith('--')) {
      if (name.endsWith(' ')) return function (condition) {
        return (0, _jsman.default)({
          type: _symbols.default.MOD,
          name: name.trim(),
          condition: condition
        });
      };else {
        throw new Error('[JSVN] Static local modifiers not supported. Add a space to the end of the modifier name and specify the condition as in the example: "[$$`--my-mod `(it=>it.condition)]"'); //return jsman_$$({ type: symbols.MOD, name });
      }
    } else {
