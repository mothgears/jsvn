...
var _childrenType = new WeakMap();

var _subclasses = new WeakMap();
...
var SourceNode = /*#__PURE__*/function () {
  ...
  function SourceNode() {
    ...
    _childrenType.set(this, {
      writable: true,
      value: null
    });

    _subclasses.set(this, {
      writable: true,
      value: {}
    });
...
var _getBaseIndex = function _getBaseIndex(key, children, baseSubclasses) {
  var baseName = null;

  if (key.base) {
    if (!key.base.length) baseName = key.name;else if (baseSubclasses[key.name]) {
      throw new Error("[JSVN] The overriding node \"".concat(key.name, "\" must not have any other bases"));
    }
  }

  if (baseName) {
    return children.findIndex(function (child) {
      if (child instanceof SourceNode) return _classPrivateFieldGet(child, _nodeName) === baseName;
      return false;
    });
  } else return -1;
};
  ...
  if (_classPrivateFieldGet(this, _children)) {
    ...
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
        ...
        var rendered = child; //text

        if (child instanceof SourceNode) rendered = child.render.apply(child, [render].concat(_toConsumableArray(envs)));else if (typeof child === 'function') rendered = child.apply(void 0, _toConsumableArray(envs)); //lambda text
            ...
            if (component instanceof _$$.default.View) rendered = component.render(render, props.apply(void 0, _toConsumableArray(envs)));else rendered = {
              JSVNContainer: true,
              component: component,
              props: props.apply(void 0, _toConsumableArray(envs))
            }; //external component
          }
...
var _parseBaseItem2 = function _parseBaseItem2(baseItem, baseNode, dependencies, parentData) {
  ...
  if (baseItem instanceof SourceNode || baseItem.isSubNode) {
    ...
    if (!_classPrivateFieldGet(this, _classes).includes(baseNode.className)) _classPrivateFieldGet(this, _classes).push(baseNode.className);
...
var _basedOn2 = function _basedOn2(baseNode) {
  ...
  _classPrivateFieldSet(this, _subclasses, _objectSpread({}, _classPrivateFieldGet(baseNode, _subclasses)));

  _classPrivateFieldSet(this, _childrenType, _classPrivateFieldGet(baseNode, _childrenType));
...
var _parseBodyItem2 = function _parseBodyItem2(css, key, value, selector, isRootNode, subclasses, baseViewName, typeMixing, dependencies, children) {
  if (key === '$') key = {
  ...
  if (parser) {
    if (typeof parser === 'string') {
      if (subclasses[parser]) throw new Error("[JSVN] Duplicate class name \"".concat(parser, "\". If you need to set common styles for multiple nodes, inherit nodes from a local class."));
      subclasses[parser] = {
        isNode: false
      };
  ...
  if (_typeof(key) === 'object') {
    ...
    if (key.type === _symbols.default.SOURCE) {
      //sourceNode
      if (children) {
        if (key.isSimple) typeMixing.haveSimple = true;else typeMixing.haveSymbol = true; //It's override some node?

        var basePointer = _classStaticPrivateMethodGet(SourceNode, SourceNode, _getBaseIndex).call(SourceNode, key, _classPrivateFieldGet(this, _children), _classPrivateFieldGet(this, _subclasses)); //console.log(`${this.className}/${key.name} : ${basePointer}`);


        if (key.name) {
          //Is unique child name
          if (subclasses[key.name] || basePointer < 0 && _classPrivateFieldGet(this, _subclasses)[key.name]) {
            throw new Error("[JSVN] Duplicate node name \"".concat(key.name, "\". If you need to set common styles for multiple nodes, inherit nodes from a local class."));
          }
        ...
        var childNode = new SourceNode(dependencies, getCSS, value, key.base, key.name, selector, this.viewName, {
          subclasses: _classPrivateFieldGet(this, _subclasses),
    ...
    if (key.type === _symbols.default.IF) {//...
    }
  } //ERROR
...
var _parseContent2 = function _parseContent2(dependencies, content, selector, isRootNode, baseViewName) {
  ...
  var subclasses = {};
  var typeMixing = {
    haveSimple: _classPrivateFieldGet(this, _childrenType) === childrenTypes.SIMPLE,
    haveSymbol: _classPrivateFieldGet(this, _childrenType) === childrenTypes.SYMBOL
  };
  var contentEntries;
  if (Array.isArray(content)) contentEntries = content;else contentEntries = Object.entries(content);
  var children = _classPrivateFieldGet(this, _children) ? [] : null;
  ...
  try {
    for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
      ...
      if (!_classPrivateMethodGet(this, _parseBodyItem, _parseBodyItem2).call(this, css, key, value, selector, isRootNode, subclasses, baseViewName, typeMixing, dependencies, children)) {
  ...
  if (children) {
    ...
    try {
      var _loop = function _loop() {
        ...
        var originIndex = _classPrivateFieldGet(_this, _children).findIndex(function (c) {
          if (c instanceof SourceNode && child instanceof SourceNode) {
            return _classPrivateFieldGet(c, _nodeName) === _classPrivateFieldGet(child, _nodeName);
          }
        ...
        if (originIndex >= 0) {
          var block = _classPrivateFieldGet(_this, _children).splice(originIndex);

          block[0] = child;
          sortedChildren.push.apply(sortedChildren, _toConsumableArray(block.reverse()));
    ...
    (_classPrivateFieldGet8 = _classPrivateFieldGet(this, _children)).push.apply(_classPrivateFieldGet8, _toConsumableArray(sortedChildren.reverse()));
  }
  ...
  for (var _i7 = 0, _Object$keys = Object.keys(subclasses); _i7 < _Object$keys.length; _i7++) {
    var scName = _Object$keys[_i7];
    ...
    _classPrivateFieldGet(this, _subclasses)[scName] = subclasses[scName];
  }

  if (typeMixing.haveSymbol) _classPrivateFieldSet(this, _childrenType, childrenTypes.SYMBOL);else if (typeMixing.haveSimple) _classPrivateFieldSet(this, _childrenType, childrenTypes.SIMPLE);
  return css;
};